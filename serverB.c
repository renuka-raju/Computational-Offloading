...
int main(void)
{
	struct sockaddr_in server_b_udp_addr;	/* server B address */
	...
	/* bind the socket to localhost and a specific port 22557*/
	memset((char *)&server_b_udp_addr, 0, sizeof(server_b_udp_addr));
	server_b_udp_addr.sin_family = AF_INET;
	server_b_udp_addr.sin_addr.s_addr = inet_addr(LOCALHOST);
	server_b_udp_addr.sin_port = htons(MYPORT);

	if (bind(fd, (struct sockaddr *)&server_b_udp_addr, sizeof(server_b_udp_addr)) < 0) {
		perror("bind failed");
		return 0;
	}
	...
	while(1) {
	   ...
	   char data[5][100];
	    ...
	    tok = strtok(line, ",");
	    if(atoi(tok)==linkid){
	    	// printf("\nBackend server B found the link..");
	    	found=1;
	    	int i=0;
